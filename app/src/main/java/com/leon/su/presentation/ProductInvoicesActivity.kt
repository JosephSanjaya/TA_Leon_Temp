...
class ProductInvoicesActivity : AppCompatActivity(R.layout.activity_tab) {
            ...
            adapter = object : FragmentStateAdapter(supportFragmentManager, lifecycle) {
                override fun createFragment(position: Int): Fragment {
                    return when (position) {
                        0 -> ProductListFragment.newInstance(Product.Type.MAKANAN)
                        1 -> ProductListFragment.newInstance(Product.Type.MINUMAN)
                        else -> ProductListFragment.newInstance(Product.Type.LAINNYA)
                    }
                }
